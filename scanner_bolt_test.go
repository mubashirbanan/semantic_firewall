...
func TestNewBoltScanner(t *testing.T) {
	...
	scanner, err := NewBoltScanner(dbPath, DefaultBoltScannerOptions())
	if err != nil {
...
func TestBoltScannerScanTopologyFuzzy(t *testing.T) {
	...
	opts := DefaultBoltScannerOptions()
	opts.EntropyTolerance = 1.0 // Wider tolerance for fuzzy matching

	scanner, err := NewBoltScanner(dbPath, opts)
	if err != nil {
		t.Fatalf("NewBoltScanner() error = %v", err)
	}
	defer scanner.Close()
...
func TestBoltScannerExportToJSON(t *testing.T) {
	...
	scanner, _ = NewBoltScanner(dbPath, DefaultBoltScannerOptions())
	defer scanner.Close()
...
func TestFormatEntropyKey(t *testing.T) {
	...
	for _, tc := range tests {
		result := formatEntropyKey(tc.entropy, tc.id)
